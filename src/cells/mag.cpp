...
namespace cells{
   ...
   if(micromagnetic::discretisation_type != 1){
      ...
      // Ensure mag_array is correctly sized
      ...
      for(int i = 0; i < num_local_atoms; ++i) {
         ...
         if (cell >= 0 && cell < cells::num_cells) {
            ...
            std::cerr << "Error: Cell index out of bounds: " << cell << std::endl;
          //  MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
         }
      }

      #ifdef MPICF
      int global_size = cells::mag_array_x.size();
      MPI_Allreduce(MPI_IN_PLACE, &cells::mag_array_x[0], global_size, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
      MPI_Allreduce(MPI_IN_PLACE, &cells::mag_array_y[0], global_size, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
      MPI_Allreduce(MPI_IN_PLACE, &cells::mag_array_z[0], global_size, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
      #endif
   }
